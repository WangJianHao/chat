package com.sen.chat.common.utils;
...
/**
 * Redis工具类，封装一些操作Redis的接口
 *
 * @description: Redis工具类，封装一些操作Redis的接口
 * @author: sensen
...
public class RedisUtils {
     ...
     * @param time 时间
     */
    public static int integerInc(String key, int time, TimeUnit timeUnit) {
        RedisScript<Long> redisScript = new DefaultRedisScript<>(LUA_INCR_EXPIRE, Long.class);
        Long result = stringRedisTemplate.execute(redisScript, Collections.singletonList(key), String.valueOf(timeUnit.toSeconds(time)));
        try {
